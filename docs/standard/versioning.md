# Versioning

Neptune. Signal uses semantic versioning for the standard.

```text
MAJOR.MINOR.PATCH
```

## Standard Versions

```text
0.2.0  Material-compatible foundations, color roles, surfaces, light/dark/black appearance roles, component anatomy, and SDK theme roles.
0.1.1  Information presentation update for critical banking values.
0.1.0  Initial public standard for mobile banking foundations.
0.3.0  KMP Compose alpha.
0.4.0  Figma component library alpha.
1.0.0  Stable mobile banking standard.
2.0.0  Internet banking and large-screen standard.
```

## Demo Versions

Demos version separately because they are examples, not the standard itself.

```text
Mobile Banking Demo 0.2.0
Internet Banking Demo 0.1.0
Merchant Portal Demo 0.1.0
Admin Console Demo 0.1.0
```

## Breaking Changes

Breaking changes include:

- Renaming public tokens.
- Changing type role meaning.
- Changing component anatomy.
- Changing navigation roots.
- Removing required accessibility behavior.
- Changing KMP API names after release.

Visual refinements that preserve names and behavior are minor or patch changes.
